...
CREATE PROCEDURE aplicar_taxa_salario(
    IN p_id_funcionario INT, 
    IN p_taxa DECIMAL(10,2)
)
BEGIN
    UPDATE funcionarios
    SET salario = salario + (salario * p_taxa)
...
/*. Crie uma tabela chamada hora_extra. Essa tabela deverá ter o identificador do funcionário e
...
CREATE PROCEDURE calcular_horas_excedentes(IN p_id_funcionario INT)
BEGIN
    DECLARE total_horas INT;

    SELECT SUM(horas_trabalhadas) INTO total_horas
    FROM trabalha
    WHERE id_funcionario = p_id_funcionario;

    IF total_horas > 40 THEN
        IF EXISTS (SELECT 1 FROM hora_extra WHERE id_funcionario = p_id_funcionario) THEN
            UPDATE hora_extra
            SET horas_excedentes = horas_excedentes + (total_horas - 40)
            WHERE id_funcionario = p_id_funcionario;
        ELSE
            INSERT INTO hora_extra (id_funcionario, horas_excedentes)
            VALUES (p_id_funcionario, total_horas - 40);
        END IF;
    END IF;
END $$
